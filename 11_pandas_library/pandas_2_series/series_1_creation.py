...
# CREATING SERIES FROM AN EXISTING SERIES USING THE COPY METHOD:
"""
One way of creating a new series is by copying an existing pandas series using the copy() 
method. The copy() method is a built in method of the pandas library and is used to create a
deep copy(by default). Hence, any changes to the copied series will not be reflected to the 
original 
series. Please refer to the below code:
"""
copied_series = series_mcs.copy()
print(copied_series)

"""
It is also possible to create a shallow copy of an existing series using the copy() method. 
This can be done by passing argument 'deep=False'. In such a case the deep copy feature of 
the copy() method is turned off.
"""
